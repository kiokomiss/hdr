...
struct PipeOpener {
    PipeOpener(const char* cmd, const char* mode) : f(_popen(cmd, mode)) {
        // nothing
    }
    ~PipeOpener() {
        if (f != nullptr) {
...
bool load_raw(const std::string &filename, uint16_t* data, int width, int height) {
    ...
    Internal::PipeOpener f(("C:/HDR/hdr-plus/tools/dcraw-9.27-ms-32-bit.exe -c -D -6 -W -g 1 1 " + filename).c_str(), "rb");
